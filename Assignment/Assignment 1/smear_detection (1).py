...
def smear_detection(path):
    ...
    cv2.waitKey()
    cv2.imwrite('Average_Image.jpg',numpy_array)

    numpy_array=cv2.imread("Average_Image.jpg",0)
    image_mask = numpy_array.astype(npy.uint8)

    #Gaussian Filtering: Smoothen,Sharpen and removal of noise 
    gaussian_image = scindi.gaussian_filter(image_mask, (10,10))
    cv2.imshow('Gaussian Filter Image',gaussian_image)
    cv2.waitKey()
    cv2.imwrite('Gaussian_Filter_Image.jpg',gaussian_image)
    ...
    cv2.waitKey()
    cv2.imwrite('Adaptive_Threshold_Image.jpg',threshold_average_image)

    #Finding the outer edges of the Smear
    ...
    #Draw contours to identify smear
    ...
    cv2.imwrite("Final_Output_Image.jpg",average_image)
    cv2.waitKey()         
